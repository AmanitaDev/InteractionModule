...
namespace IPD
{
    ...
    public class InputPromptImage : MonoBehaviour
    {
        ...
        [SerializeField] private DisplaySettingsModel settings;
        ...
        public async void Initialize(DisplaySettingsModel newSettings)
        {
            ...
            if (action == null)
            {
                ...
                LogIssue(
                    $"The action '{settings.Action}' was not found in the current action map '{player.currentActionMap.name}'",
                    false);
                return;
            }
            ...
            if (action.controls.Count < 1)
            {
                ...
                LogIssue(
                    $"The action '{settings.Action}' has no controls bound for the control scheme '{player.currentControlScheme}'",
                    settings.IsUnassignedDisplayed);
                return;
            }
            ...
            else if (!string.IsNullOrEmpty(settings.Composite))
            {
                ...
                if (control == null)
                {
                    ...
                    if (altBindingTracker > 0)
                    {
                        ...
                        LogIssue(
                            $"The action '{settings.Action}' only has {altBindingTracker} composite '{settings.Composite}' binding(s) for the control scheme '{player.currentControlScheme}'",
                            settings.IsUnassignedDisplayed);
                    }
                    else
                    {
                        ...
                        LogIssue(
                            $"The action '{settings.Action}' does not contain the composite '{settings.Composite}' binding for the control scheme '{player.currentControlScheme}'",
                            settings.IsUnassignedDisplayed);
                    }
            ...
            else
            {
                ...
                if (settings.AltBindingIndex >= action.controls.Count)
                {
                    ...
                    LogIssue(
                        $"The action '{settings.Action}' only has {action.controls.Count} control(s) for the control scheme '{player.currentControlScheme}'",
                        settings.IsUnassignedDisplayed);
                    return;
                }
